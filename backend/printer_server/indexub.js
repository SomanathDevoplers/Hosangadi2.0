...
const { options, page } = require("pdfkit");
...
function Invoice(BillNumber, Date, customerName, InvoiceData, BillTotal, old_bal,oldBalData,page, res){
  ...
  height = 85+ numberOfItems*20.5+50
    ...
    if(old_bal === 'True'){
    doc
        .text(`ಹಳೆ ಬಾಕಿ : ₹ ${Number(oldBalData['old_bal']).toFixed(2)}`,10,position+10,{ align: "left" })
...
function voucher( Date, customerName, oldBalData , res){
  ...
  doc
      ...
      .text(`ಉಳಿದ ಬಾಕಿ : ₹ ${Number(oldBalData['remaining_bal']).toFixed(2)}`,0,40,{ align: "right" })
...
function barcode_50_25(image,name,mrp,cp,sp,count){
  ...
  doc.pipe(createWriteStream(path.join(homeDir,'Hosangadi2.0','barcode.pdf')));
  for(i = 0; i<count;i++)
  {   
  ...
  for(i=0;i<noOfPurchase;i++)
  {
    ...
    temp = []
      ...
      for(k=0;k<prodId.length;k++)
      {
        ...
        console.log(result1 , "SELECT stk_cost,prod_gst,prod_cess FROM somanath20"+dbYear+".stocks,somanath.products where stk_prod_id = "+prodId[k]+" and stk_prod_id = prod_id   and stk_pur_id = '"+allPurchases[i]["pur_id"]+"';");
...
XLSX.writeFile(newBook,"Purchase_"+month+".xlsx");
}
else{
  res.sendStatus(201)
}
...
app.post('/PrintInvoice' , files.single('file') ,(req,res) =>{
    ...
    if(req.query.range != '')
      {
        printOptions.push('-P '+req.query.range)
    ...
    if(req.query.even_odd != '-1')
      {
        ...
        if (req.query.even_odd == '0')
            {   
                ...
                for(x=1;x<=req.query.total_pages ; x++)
                    if(x%2)
                        page_string += x + ","
            }
        else
        ...
        page_string = page_string.slice(0,-1)

        printOptions.push('-P '+page_string)
      }
    


    print.print(path.join(homeDir , "invoice.pdf") , "EPSON" ,  printOptions )
